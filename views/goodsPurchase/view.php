	...
	<form name="theForm" method="post" action="<?php echo lib_functions::url('goodsPurchase/edit/'.$response->id);?>" width="60%" onsubmit="return form_submit('theForm')">
		...
		<table class="table-div" cellspacing="0" cellpadding="0" border="1">
			<tr>
				...
				<td>
					<input type="text" name="ywrq" class="required" readOnly="readOnly" value="<?php echo date('Y-m-d H:i',$response->result['ywrq']);?>"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<select name="supplier"  class="required" disabled="true">
				...
				<td>
					<select name="warehouse"  class="required" disabled="true">
						<option value="">请选择</option>
			...
			<tr>
				...
				<td><input type='text' name='maker'  readOnly="readOnly" value="<?php echo date('Y-m-d H:i:s',$response->result['create_time']);?>" readOnly="readOnly" />
			</tr>
			<tr>
				...
				<?php 
					if($response->result['status'] == 1) {
						echo '已验收';
					}elseif($response->result['status'] == 0) {
						echo '未验收';
					}
				?>
				</td>
				...
				<td>
					<input type='text' name='desc'  readOnly="readOnly"  title="<?php echo $response->result['description'];?>" value="<?php echo $response->result['description'];?>" />
				</td>
			</tr>
			<tr>
